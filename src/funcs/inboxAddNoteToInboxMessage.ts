 ...
 */
export function inboxAddNoteToInboxMessage(
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.AddNoteToInboxMessageRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.AddNoteRequest, { explode: true });
  ...
  const path = pathToFunc("/inboxmessage/{uuid}/notes.json")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(201, components.SuccessResponse$inboundSchema),
