 ...
 */
export function inboxGetInboxMessage(
  client: ServiceM8Core,
  request: operations.GetInboxMessageRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.InboxMessageDetail,
    | errors.ErrorT
    | ServiceM8Error
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetInboxMessageRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/inboxmessage/{uuid}.json")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.InboxMessageDetail$inboundSchema),
