 ...
 */
export function suppliersCreateSuppliers(
  client: ServiceM8Core,
  request: components.SupplierCreate,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.CreateSuppliersResponse,
    | errors.ErrorT
...
> {
  const parsed = safeParse(
    request,
    (value) => components.SupplierCreate$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/supplier.json")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.CreateSuppliersResponse$inboundSchema, {
      hdrs: true,
      key: "Result",
    }),
