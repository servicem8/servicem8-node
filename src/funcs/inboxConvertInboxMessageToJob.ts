 ...
 */
export function inboxConvertInboxMessageToJob(
  client: ServiceM8Core,
  request: operations.ConvertInboxMessageToJobRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.ConvertToJobResponse,
    | ServiceM8Error
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.ConvertInboxMessageToJobRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.ConvertToJobRequest, {
    explode: true,
  });
  ...
  const path = pathToFunc("/inboxmessage/{uuid}/convert-to-job.json")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(201, components.ConvertToJobResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
