 ...
 */
export function inboxAttachInboxMessageToJob(
  client: ServiceM8Core,
  request: operations.AttachInboxMessageToJobRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.AttachToJobResponse,
    | errors.ErrorT
    | ServiceM8Error
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.AttachInboxMessageToJobRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.AttachToJobRequest, {
    explode: true,
  });
  ...
  const path = pathToFunc("/inboxmessage/{uuid}/attach-to-job.json")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.AttachToJobResponse$inboundSchema),
