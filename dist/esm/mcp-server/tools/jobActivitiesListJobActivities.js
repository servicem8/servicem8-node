/*
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 */
import { jobActivitiesListJobActivities } from "../../funcs/jobActivitiesListJobActivities.js";
import { formatResult } from "../tools.js";
export const tool$jobActivitiesListJobActivities = {
    name: "job-activities-list-job-activities",
    description: `List all Job Activities


			
#### Filtering
This endpoint supports result filtering. For more information on how to filter this request, [go here](/docs/filtering).
			
			
#### OAuth Scope
This endpoint requires the following OAuth scope **read_schedule**.

			`,
    tool: async (client, ctx) => {
        const [result, apiCall] = await jobActivitiesListJobActivities(client, { fetchOptions: { signal: ctx.signal } }).$inspect();
        if (!result.ok) {
            return {
                content: [{ type: "text", text: result.error.message }],
                isError: true,
            };
        }
        const value = result.value;
        return formatResult(value, apiCall);
    },
};
//# sourceMappingURL=jobActivitiesListJobActivities.js.map